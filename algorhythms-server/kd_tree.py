...
Point = TypeVar('Point', bound=Dict[str, float])
class PointKeys(str):
    pass
...
class KDTree(Generic[Point]):
    ...
    def __init__(self, points: List[Point]) -> None:
        ...
        def build_tree(current_points: List[Point], depth: int = 0) -> Optional[KDNode[Point]]:
    ...
    def nearest_neighbor(self, target: Point, limit: int = 1, dist_func: Callable[[float, float], float] = lambda x1, x2: (x1 - x2)**2) -> List[Optional[Point]]:
        if self.root is None or limit <= 0:
            return [None] * limit

        cumulative_dist_func = lambda p1, p2: sum(dist_func(p1[k], p2[k]) for k in p1)
        ...
        def process_branch(root: Optional[KDNode]) -> None:
            ...
            dist_to_plane = dist_func(target[axis], root.point[axis])
